...
func testDefaultLicenseCase(t *testing.T, context spec.G, it spec.S) {
	...
	context("failure cases", func() {
		...
		context("the artifact does not contain a license", func() {
			it("returns an error and exits non-zero", func() {
				_, err := licenseRetriever.LookupLicenses("dependency", fmt.Sprintf("%s/no-license.tgz", mockServer.URL))
				Expect(err).To(HaveOccurred())
				Expect(err).To(MatchError(ContainSubstring("failed to detect licenses: no license file was found")))
			})
		})
	})
}
