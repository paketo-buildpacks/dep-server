	...
	context("failure cases", func() {
		...
		context("the bundler artifact does not contain a license", func() {
			it("returns an error and exits non-zero", func() {
				_, err := licenseRetriever.LookupLicenses("bundler", fmt.Sprintf("%s/no-license.tgz", mockServer.URL))
				Expect(err).To(HaveOccurred())
				Expect(err).To(MatchError("failed to detect licenses: no license file was found"))
			})
		})
	})
}
